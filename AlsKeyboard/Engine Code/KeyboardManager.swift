...
class KeyboardManager: NSObject {
    ...
    init(withDelegate: KeyboardDelegate) {
        ...
        self.commandMapping = ["ERERSM": "C", "SMERSM": "U", "BBERER": "J",
                               "ERBBER": "D", "BBBBER": "M", "ERSMER": "G", "ERERBB": "B",
                               "SMBBSM": "X", "ERBBBB": "E", "SMSMER": "Y", "BBBBBB": "N", "ERBBSM": "F",
                               "SMBBER": "V", "BBSMER": "P", "ERERER": "Z", "SMERBB": "T", "ERSMBB": "H",
                               "BBERSM": "L", "SMSMSM": "Q", "SMERER": "S",  "ERSMSM": "I",
                               "BBSMSM": "R",
                               "BBSMBB": "1", "SMBBBB": "2", "SMSMBB": "A", "BBBBSM": "O", "BBERBB": "K"]
    ...
    private func analyzeCommandToGenerateText() {
        ...
        if length < 3 {
            ...
            if let mappingValue = self.commandMapping[mappingKeyString] {
                ...
                if mappingValue == "DELETE" {
                    self.currentText = String(self.currentText.characters.dropLast())
                }
                else {
                    if mappingValue == "1" {
                        let request = NSMutableURLRequest(url: NSURL(string: "http://192.168.1.46:8000/start")! as URL)
                        request.httpMethod = "GET"
                        let dataTask = URLSession.shared.dataTask(with: request as URLRequest, completionHandler: { (data, response, error) -> Void in})
                        dataTask.resume()
                    }
                    if mappingValue == "2" {
                        let request = NSMutableURLRequest(url: NSURL(string: "http://192.168.1.46:8000/stop")! as URL)
                        request.httpMethod = "GET"
                        let dataTask = URLSession.shared.dataTask(with: request as URLRequest, completionHandler: { (data, response, error) -> Void in})
                        dataTask.resume()
                    }
